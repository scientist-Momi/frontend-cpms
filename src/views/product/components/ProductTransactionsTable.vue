      ...
      <tbody class="text-sm">
        <template v-for="tx in transactions" :key="tx.transactionId">
          <!-- Main transaction row -->
            <tr @click="router.push({ name: 'TransactionView', params: { id: tx.transactionId }})" class="border-b border-gray-200 cursor-pointer hover:bg-gray-50 transition">
          ...
          <template
          v-for="detail in tx.transactionDetails.filter(d => d.productId.toString() === selectedProductId.toString())"
            :key="detail.detailId"
          >
